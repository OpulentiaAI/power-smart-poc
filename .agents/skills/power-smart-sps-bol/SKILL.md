...
Follow [show your work](../power-smart-show-your-work/SKILL.md) for the whole run: narrate each phase before you start it, record every transfer in the transcript document, and show the transforms as tables and previews, not just file paths.
...
4. The transcript shows the validation table, the arithmetic table, and at least one BOL preview, each with an annotation.
...
This phase is done when both input files exist at confirmed paths.

## Phase 2. Generate

This runs offline and needs no SPS API access:
...
Record one transcript entry per transfer in the chain:
